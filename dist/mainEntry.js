...
var sub_process = [];
var get_list = (callback) => {
  axios_default.get(`${host}/codes`).then((resp) => {
    callback(resp);
  }).catch((err) => console.error(err));
};
import_electron.ipcMain.on("test", (event, count) => {
  get_list((resp) => console.log("all:", resp));
});
import_electron.ipcMain.on("showRecorder", async (event, hello, count) => {
  mainWindow.webContents.send("CODE_CHANGE", "init MESSAGE");
  const proc = (0, import_child_process.spawn)("node", [
    // "--inspect-brk",
    "./src/main/pw.js",
    "http://douban.com"
  ], {
...
import_electron.ipcMain.on("close_all", (event, count) => sub_process.forEach((p) => p.kill(9)));
import_electron.ipcMain.on("upload", (event, code) => {
  console.log("code in upload[main]", code);
  axios_default.post(`${host}/code`, { code }, { responseType: "json" }).then((resp) => console.log("axios get", { data: resp.data })).catch((err) => console.error(err));
  get_list((resp) => {
    mainWindow.webContents.send("REFRESH_LIST", resp.data);
  });
});
